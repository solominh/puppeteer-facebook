...
const _getTitle = (full, resolve) => {
  ...
  if (title === null) window.setTimeout(_getTitle, 200, full, resolve);
  else resolve(title);
};
// promise of a title
const getTitle = full =>
...
const processSubInfo = async result => {
  ...
  console.log({ result, titleP, subs });
...
const getSubsFromCache = () => {
  ...
  if (subCache.hasOwnProperty(id)) return subCache[id];

  let newID = undefined;
  try {
    newID = unsafeWindow.netflix.falcorCache.videos[id].current.value[1];
  } catch (ignore) {}
  if (typeof newID !== 'undefined' && subCache.hasOwnProperty(newID)) return subCache[newID];
...
const pickFormat = formats => {
  ...
  if (subFormat === DFXP) preferred.push(preferred.shift());
...
const _download = async _zip => {
  ...
  if (langs === '') filteredLangs = Object.keys(subs);
    ...
    const regularExpression = new RegExp(
      '^(' + langs.replace(/\[/g, '\\[').replace(/\]/g, '\\]').replace(/\-/g, '\\-').replace(/\s/g, '').replace(/,/g, '|') + ')'
    );
    filteredLangs = [];
  ...
  if ((await Promise.race([progress.stop, {}])) === STOP_THE_DOWNLOAD) stop = true;
  progress.destroy();
      ...
      if (data && typeof data.url === 'string' && data.url.search(MANIFEST_PATTERN) > -1) {
        for (let v of Object.values(data)) {
          try {
            ...
            if (v.showAllSubDubTracks != null && forceSubs) v.showAllSubDubTracks = true;
          } catch (e) {
